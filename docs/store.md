...
> **Status:** the base `Store` is built. This file is how to *use* it and what constrains code around it. Signatures live in the source (`src/lib/core/store.ts`); behaviour is pinned by its own suite (§5).
...
- **Library widgets never use a store — not even a private instance of the base `Store` as local state.** A widget with state uses a small listener array or its own `Evented` subclass; it must not import, reference, or know about any store.
...
API surface: `get`, `getAll`, `set`, `update`, `batch`, `subscribe`, `subscribeMany`, each subscription accepting an `{ immediate }` option.
...
- **Do not read the store in a field initializer** (`#model = new Model(derive(store...))`). Field init runs at element **construction** (skill §4), which can precede store population → an empty model. Instead: **construct the model empty; populate it via the same sync path used for every later update.**
