...
/** Options accepted by {@link toCql}. */
export interface CompileOptions {
  /** Allowlist of queryable columns; clauses on any other field throw. */
  knownFields?: ReadonlySet<string>;
}

/** Compiles a {@link FilterNode} to an OGC CQL expression (GeoServer `CQL_FILTER`). */
export function toCql(node: FilterNode, options: CompileOptions = {}): string {
  if (node.kind === 'group') {
    if (node.clauses.length === 0) throw new Error('toCql: empty filter group');
    const parts = node.clauses.map(clause => toCql(clause, options));
    return `(${parts.join(` ${node.op.toUpperCase()} `)})`;
  }
  return clauseToCql(node, options);
}

function clauseToCql(clause: FilterClause, options: CompileOptions): string {
  const { field, op } = clause;
  assertQueryField(field, options.knownFields, 'toCql');
  switch (op) {
    ...
    case 'in': {
      ...
      if (!Array.isArray(values) || values.length === 0) {
        throw new Error(`toCql: "in" clause on "${field}" needs a non-empty array value`);
      }
...
function requireScalar(clause: FilterClause): string | number {
  ...
  if (typeof value !== 'string' && typeof value !== 'number') {
    throw new Error(`toCql: "${op}" clause on "${field}" needs a scalar value`);
  }
  return value;
}

/** Numbers bare; strings single-quoted with `'` doubled (`O'Neil` → `'O''Neil'`). */
